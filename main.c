...
int saisie_mode;
    ...
    if (mode == 2) {
        ...
        maConfig.nb_total_cartes =  TOTAL_CARTES  ; // Taille classique
    }
...
//2.On crée un tableau de joueurs de la taille choisie par le joueur (allocation dynamique) et on vérifie que l'allocation a réussi
// Allocation dynamique du tableau de joueurs

// On utilise unsigned int pour rassurer le compilateur en prouvant que le nombre n'est pas négatif
Joueur *joueurs = malloc((unsigned int)nb_joueurs * sizeof(Joueur));

// Vérification de sécurité avec malloc
if (joueurs == NULL) {
    printf(ROUGE "Erreur critique : Impossible d'allouer la mémoire pour les joueurs.\n" RESET);
    return 1; // On quitte le programme  en cas d'échec
}
