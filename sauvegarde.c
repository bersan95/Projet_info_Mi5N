...
void sauvegarder_scores(Joueur joueurs[], int nb_joueurs) {
    ...
    while (!saisie_valide) {
        ...
        if (strcmp(reponse, "oui") == 0 || strcmp(reponse, "Oui") == 0 || strcmp(reponse, "OUI") == 0) {
           ...
           if (fichier != NULL) {
                fprintf(fichier ,  "----------------------📊 CLASSEMENT FINAL 📊----------------------\n" );
                
                for (int i = 0; i < nb_joueurs; i++) {
                    
                    // On ajoute un classement
                    if (i == 0) {
                        fprintf(fichier, "1er  🥇 : %s ( %d points)\n", joueurs[i].nom, joueurs[i].score_partie);
                    } else if (i == 1) {
                        fprintf(fichier, "2eme 🥈 : %s ( %d points)\n", joueurs[i].nom, joueurs[i].score_partie);
                    } else if (i == 2) {
                        fprintf(fichier, "3eme 🥉 : %s ( %d points)\n", joueurs[i].nom, joueurs[i].score_partie);
                    } else {
                        fprintf(fichier, "%deme    : %s ( %d points)\n", i + 1, joueurs[i].nom, joueurs[i].score_partie);
                    }
                }
